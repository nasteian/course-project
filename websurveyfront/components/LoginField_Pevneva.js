...
export default class LoginField_Pevneva extends React.Component {  
  ...
  validate = async (login) => {
    this.setError('')
    if (login === '') {
      this.setError(Strings_Pevneva.LOGIN_EMPTY)
      return true
    }
    ...
    switch(this.props.type) {
      case LoginFieldTypes.REGISTER:
        if (isLoginExist) this.setError(Strings_Pevneva.LOGIN_EXIST)
        else this.setError(''); 
        break

      case LoginFieldTypes.LOGIN:
        if (!isLoginExist) this.setError(Strings_Pevneva.LOGIN_NOT_EXIST)
        else this.setError(''); 
        break
    }
